...
function Badge({ variant: _variant, className, ...props }) {
  const badgeClassName = ["inline-flex items-center rounded-full px-3.5 py-1.5", className]
    .filter(Boolean)
    .join(" ");
...
function ImpactHero({ item }) {
  const delta = item.projected - item.score;
  const pillarBreakdown = sections.filter((section) => section.id !== item.id).map((section) => ({ id: section.id, title: section.shortTitle, uplift: section.projected - section.score, color: section.color }));
  return <Card className="rounded-[32px] border-zinc-200 shadow-sm"><CardContent className="p-7"><div className="max-w-3xl"><div className="flex items-center gap-3"><Badge className="rounded-full bg-rose-100 text-rose-700 hover:bg-rose-100">Impact score</Badge><Badge variant="outline" className="rounded-full">Primary value driver</Badge></div><h2 className="mt-4 text-4xl font-semibold tracking-tight">Highlight the impact Channel Factory can create</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-600">This lead module brings the proprietary impact score above the rest of the audit so it becomes the headline story: current impact from historical Meta activity, projected impact with Channel Factory products, and the account score that supports that uplift.</p><div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricTile label="Current impact" value={item.score} /><MetricTile label="Projected impact" value={item.projected} /><div className="rounded-3xl border border-zinc-200 bg-zinc-50 p-4 shadow-sm"><div className="text-[11px] uppercase tracking-[0.16em] text-zinc-500">Impact delta</div><div className="mt-2 text-2xl font-semibold text-emerald-600">+{delta}</div><div className="mt-1 text-sm text-zinc-500">Modeled uplift opportunity</div></div><div className="rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm"><div className="text-[11px] uppercase tracking-[0.16em] text-zinc-500">Projected improvement mix</div><div className="mt-3 space-y-2">{pillarBreakdown.map((pillar) => <div key={pillar.id} className="flex items-center justify-between gap-3 text-sm"><div className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: pillar.color }} aria-hidden /><span className="text-zinc-600">{pillar.title}</span></div><span className="font-medium text-zinc-900">+{pillar.uplift}</span></div>)}</div></div></div></div></CardContent></Card>;
}

function getAssessmentResult(detail) {
...
function DefaultExpandedPanel({ section }) {
  ...
  return <div className="rounded-[28px] border border-zinc-100 bg-zinc-50/70 p-5"><div className="mb-4 flex items-center justify-between gap-4"><div><p className="text-sm font-medium text-zinc-900">What we would assess in this area</p><p className="mt-1 text-sm text-zinc-500">These are the diagnostic themes the audit can expand on when this score is selected.</p></div><Badge className="rounded-full border-0 text-white" style={{ backgroundColor: section.color }}>{section.details?.length || 0} diagnostic checks</Badge></div><button type="button" onClick={() => setIsExpanded((value) => !value)} className="flex w-full items-center justify-between rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-left text-sm text-zinc-600 shadow-sm transition-colors hover:bg-zinc-50"><span>{isExpanded ? "Hide assessment criteria" : "Show assessment criteria"}</span><span className="font-medium text-zinc-900">{isExpanded ? "Collapse" : "Expand"}</span></button>{isExpanded ? <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{(section.details || []).map((detail) => { const isPass = getAssessmentResult(detail) === "pass"; return <div key={detail} className={`rounded-2xl border p-4 text-sm leading-6 shadow-sm ${isPass ? "border-emerald-200 bg-emerald-50/70 text-emerald-900" : "border-rose-200 bg-rose-50/70 text-rose-950"}`}><div className="mb-3 flex items-center justify-between gap-3"><span className={`inline-flex items-center rounded-full px-3 py-1 text-[11px] font-medium uppercase tracking-[0.16em] ${isPass ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}>{isPass ? "Pass" : "Fail"}</span></div><p>{detail}</p></div>; })}</div> : null}</div>;
}
...
function SectionExpandedPanel({ section }) {
  ...
  return <div className="space-y-5"><div className="space-y-4">{leadBlock}{breakdownBlock}</div>{cfg.insights && cfg.insightIcon && (cfg.leadCards || cfg.creativeAds) ? <div className="space-y-4"><InsightList title={cfg.insightTitle || "Insights"} subtitle={cfg.insightSubtitle || ""} points={cfg.insights} icon={cfg.insightIcon} color={section.color} /><DefaultExpandedPanel section={section} /></div> : <DefaultExpandedPanel section={section} />}</div>;
}

function RadarRollup({ sections, hovered, setHovered, onSelect }) {
  const data = sections.map((s) => ({ id: s.id, subject: s.shortTitle, title: s.title, current: s.score, projected: s.projected, color: s.color, light: s.light }));
  return <Card className="overflow-hidden rounded-[28px] border-zinc-200 shadow-sm"><CardHeader className="px-6 pb-2 pt-5 sm:px-7 sm:pt-6"><div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><CardTitle className="text-xl">How the overall score rolls up</CardTitle><p className="mt-1 text-sm text-zinc-500">Hover the shape to explore each scoring dimension and compare current performance with Channel Factory upside.</p></div><div className="flex flex-wrap gap-2 text-xs"><div className="rounded-full border border-zinc-200 bg-white px-3 py-1.5 text-zinc-600">Current account shape</div><div className="rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1.5 text-zinc-600">Projected opportunity envelope</div></div></div></CardHeader><CardContent><div className="grid gap-6 lg:grid-cols-[1.25fr_0.75fr]"><div className="relative overflow-hidden rounded-[32px] border border-zinc-100 bg-[radial-gradient(circle_at_top,_rgba(255,255,255,1),_rgba(244,244,245,0.9)_45%,_rgba(255,255,255,1)_100%)] p-4 sm:p-8"><div className="pointer-events-none absolute left-1/2 top-1/2 h-56 w-56 -translate-x-1/2 -translate-y-1/2 rounded-full blur-3xl" style={{ backgroundColor: `${hovered.color}20` }} /><div className="relative mx-auto h-[420px] max-w-3xl"><ResponsiveContainer width="100%" height="100%"><RadarChart data={data} outerRadius="72%" onMouseMove={(state) => { const match = sections.find((s) => s.shortTitle === state?.activeLabel); if (match) setHovered(match.id); }} onMouseLeave={() => setHovered("impact")} onClick={(state) => { const match = sections.find((s) => s.shortTitle === state?.activeLabel); if (match) onSelect(match.id); }}><defs><linearGradient id="projectedFill" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#dbeafe" stopOpacity={0.9} /><stop offset="100%" stopColor="#e2e8f0" stopOpacity={0.5} /></linearGradient><linearGradient id="currentFill" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor={hovered.color} stopOpacity={0.35} /><stop offset="100%" stopColor="#111827" stopOpacity={0.18} /></linearGradient></defs><PolarGrid stroke="#d4d4d8" radialLines /><PolarAngleAxis dataKey="subject" tick={{ fontSize: 12, fill: "#71717a" }} /><PolarRadiusAxis domain={[0, 100]} tick={false} axisLine={false} /><Tooltip content={<RadarTooltip />} /><Radar name="Projected" dataKey="projected" stroke="#cbd5e1" fill="url(#projectedFill)" fillOpacity={0.75} strokeWidth={1.5} /><Radar name="Current" dataKey="current" stroke={hovered.color} fill="url(#currentFill)" fillOpacity={0.95} strokeWidth={2.5} /></RadarChart></ResponsiveContainer></div></div><div className="flex flex-col justify-between gap-4 rounded-[32px] border border-zinc-100 bg-white p-5 shadow-sm"><div><div className="flex items-center gap-3"><div className="h-3 w-3 rounded-full" style={{ backgroundColor: hovered.color }} /><p className="text-sm font-medium text-zinc-500">Hovered dimension</p></div><h3 className="mt-3 text-2xl font-semibold text-zinc-900">{hovered.title}</h3><p className="mt-3 text-sm leading-6 text-zinc-600">{hovered.summary}</p></div><div className="grid gap-3"><MetricTile label="Current score" value={hovered.score} /><MetricTile label="Projected score" value={hovered.projected} /><div className="rounded-3xl p-4" style={{ backgroundColor: hovered.light }}><div className="text-xs uppercase tracking-[0.16em]" style={{ color: hovered.color }}>Potential uplift</div><div className="mt-2 text-3xl font-semibold" style={{ color: hovered.color }}>+{hovered.projected - hovered.score}</div></div><button type="button" onClick={() => onSelect(hovered.id)} className="rounded-3xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-left text-sm text-zinc-600 transition-colors hover:bg-zinc-100">{hovered.id === "impact" ? "Return to the Impact lead section" : `Jump to ${hovered.title} details`}</button></div></div></div></CardContent></Card>;
}
